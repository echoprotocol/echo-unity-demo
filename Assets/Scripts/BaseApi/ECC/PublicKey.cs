...
namespace Base.ECC
{
    ...
    public sealed class PublicKey : ISerializeToBuffer, IEquatable<PublicKey>, IComparable<PublicKey>
    {
        ...
        private static PublicKey FromStringOrThrow(string publicKey, string addressPrefix = null)
        {
            if (addressPrefix.IsNull())
            {
                addressPrefix = ChainConfig.AddressPrefix;
        ...
        private PublicKey Child(byte[] offset)
        {
        ...
        public bool Equals(PublicKey other) => ToString().Equals(other.ToNullableString());
