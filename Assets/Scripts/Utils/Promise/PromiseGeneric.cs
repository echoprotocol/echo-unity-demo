...
namespace Promises
{
    ...
    public class Promise<PromisedT> : IPromise<PromisedT>, IPendingPromise<PromisedT>, IPromiseInfo
    {
        ...
        {
            (rejectHandlers ?? (rejectHandlers = new List<RejectHandler>())).Add(new RejectHandler { callback = onRejected, rejectable = rejectable });
        }
        ...
        {
            (resolveCallbacks ?? (resolveCallbacks = new List<Action<PromisedT>>())).Add(onResolved);
            (resolveRejectables ?? (resolveRejectables = new List<IRejectable>())).Add(rejectable);
        }
        ...
        {
            //Argument.NotNull( () => ex );
            if (!CurrentState.Equals(PromiseState.Pending))
            {
                throw new ApplicationException("Attempt to reject a promise that is already in state: " + CurrentState + ", a promise can only be rejected when it is still in state: " + PromiseState.Pending);
            }
        ...
        public void Resolve(PromisedT value)
        {
            if (!CurrentState.Equals(PromiseState.Pending))
            {
                throw new ApplicationException("Attempt to resolve a promise that is already in state: " + CurrentState + ", a promise can only be resolved when it is still in state: " + PromiseState.Pending);
            }
            resolveValue = value;
        ...
        /// onRejected is called on error.
        /// </summary>
        public void Done(Action<PromisedT> onResolved, Action<Exception> onRejected) => Then(onResolved, onRejected).Catch(ex => Promise.PropagateUnhandledException(this, ex));
        ...
        /// Adds a default error handler.
        /// </summary>
        public void Done(Action<PromisedT> onResolved) => Then(onResolved).Catch(ex => Promise.PropagateUnhandledException(this, ex));

        /// <summary>
        /// Complete the promise. Adds a default error handler.
        /// </summary>
        public void Done() => Catch(ex => Promise.PropagateUnhandledException(this, ex));
        ...
        /// Add a resolved callback that chains a value promise (optionally converting to a different value type).
        /// </summary>
        public IPromise<ConvertedT> Then<ConvertedT>(Func<PromisedT, IPromise<ConvertedT>> onResolved) => Then(onResolved, null);

        /// <summary>
        /// Add a resolved callback that chains a non-value promise.
        /// </summary>
        public IPromise Then(Func<PromisedT, IPromise> onResolved) => Then(onResolved, null);

        /// <summary>
        /// Add a resolved callback.
        /// </summary>
        public IPromise<PromisedT> Then(Action<PromisedT> onResolved) => Then(onResolved, null);
        ...
        public static IPromise<IEnumerable<PromisedT>> All(IEnumerable<IPromise<PromisedT>> promises)
        {
            ...
            {
                return Promise<IEnumerable<PromisedT>>.Resolved(EnumerableExtensions.Empty<PromisedT>());
            }
            ...
            promisesArray.Each((promise, index) =>
            {
                promise
                    .Catch(ex =>
                    {
                        if (resultPromise.CurrentState.Equals(PromiseState.Pending))
                        {
